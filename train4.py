...
def extract_numerical_features(word):
    """Extract comprehensive linguistic features from a word"""
    ...
    # Hyphen (code-switching indicator)
...
# Extract character n-grams using TF-IDF (reduced features for better balance)
vectorizer = TfidfVectorizer(
    analyzer='char',
    ngram_range=(2, 4),  # Bigrams to 4-grams (better than 1,3)
    max_features=300     # Reduced to balance with numerical features
)
...
# Scale numerical features to match TF-IDF range
...
# ========== 6. Train Hybrid Multinomial Naive Bayes Model ==========
print("\nTraining Hybrid Multinomial Naive Bayes model...")
model = MultinomialNB(
    alpha=0.1  # Smoothing parameter (lower = less smoothing, more reliance on actual data)
)
...
# ========== 9. Save Model and Feature Extractors ==========
with open("trained_model_hybrid.pkl", "wb") as f:
    pickle.dump((model, vectorizer, scaler), f)

print("Hybrid Multinomial Naive Bayes model, vectorizer, and scaler saved to 'trained_model_hybrid.pkl'")

# ========== 10. Comparison Summary ==========
print("\n" + "="*60)
print("CURRENT MODEL PERFORMANCE")
print("="*60)
print(f"Multinomial Naive Bayes + HYBRID: {test_acc:.2%}")
print("="*60)